...
void fifo::write (char c) {
	//if(num_elements==size) wait(read_event); TASK 2
...
char fifo::read () {
  ...
  last_time = sc_time_stamp();
	//if(num_elements==0) wait(write_event); TASK 2
...
void fifo::read_request(bool read){
   if(read){
	if(!busy){
		if(!empty()){
		  busy=true;		
...
void fifo::write_request(bool write){
    if(write){
	if(!busy){
		if(!full()){
		busy = true;
